...
import logging
logger = logging.getLogger('antiope.AWSAccount')
...
class AWSAccount(object):
    ...
    def __init__(self, account_id, config=None):
        ...
        if config is None:
            # Assume we're able to get what we need from EnvVars
            self.config = AntiopeConfig()
        else:
            self.config = config
    ...
    def get_vpcs(self, region=None):
        ...
        for v in vpc_list:
            vpc = VPC(v,  account=self, config=self.config)
            if region is None:
                output.append(vpc)
            else:
